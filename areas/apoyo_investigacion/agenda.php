    ...
    if ($accion === 'crear_evento') {
        ...
        if ($titulo) {
            ...
            $pdo->prepare("INSERT INTO sb_kanban_tareas (titulo, descripcion, color, estatus, creado_por, asignado_a, cve_area) VALUES (?,?,?,'pendiente',?,?,?)")
                ->execute([$titulo, trim(postParam('descripcion')), postParam('color','#3730a3'), $idAutor, $asig, $cveArea]);
        ...
        if ($id > 0 && trim(postParam('titulo'))) {
            ...
            $pdo->prepare("UPDATE sb_kanban_tareas SET titulo=?,descripcion=?,color=?,asignado_a=? WHERE id=?")
                ->execute([trim(postParam('titulo')), trim(postParam('descripcion')), postParam('color','#3730a3'), $asig, $id]);
...
$extraHead = '<style>
:root { --color-primary:#3730a3; --color-primary-dark:#1e1b4b; --color-primary-light:#3b82f6; --color-primary-hover:#4338ca; --color-accent:#B19A6D; }
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
    <div>
        <h2 style="font-weight:800;color:#0f172a;margin:0;"><i class="fa-solid fa-calendar-week" style="color:#3730a3;"></i> Agenda JurÃ­dico-Administrativa</h2>
        <p style="color:#64748b;margin:0;">GestiÃ³n de eventos del Ã¡rea y tablero de tareas.</p>
    </div>
    <div style="display:flex;gap:10px;">
        <button class="btn btn-primary" onclick="abrirModalCrear()" style="background:#3730a3;border-color:#3730a3;">
        ...
        <?php for ($dia = 1; $dia <= $diasEnMes; $dia++):
            ...
            <div class="calendar-cell <?= $esHoy ? 'today' : '' ?>" onclick="abrirModalCrearDesdeCelda('<?= $fechaIso ?>','<?= $fechaIso ?>')">
                ...
                <?php foreach ($calEvs[$dia] ?? [] as $ev):
                    $colorNota  = isset($ev['es_cumple']) ? 'nota-dorado' : (isset($ev['es_institucional']) ? 'nota-institucional' : 'nota-inv');
    ...
    <div class="modal">
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                ...
                <div><label class="form-label">Color</label><input type="color" name="color" value="#3730a3" class="form-control" style="height:42px;padding:4px;"></div>
            </div>
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" style="background:#3730a3;border-color:#3730a3;"><i class="fa-solid fa-check"></i> Guardar</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="modal">
        ...
        <form method="POST" id="formEditarEvento">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" style="background:#3730a3;border-color:#3730a3;"><i class="fa-solid fa-check"></i> Actualizar</button>
            </div>
        </form>
    </div>
</div>
...
function abrirModalEditar(id, titulo, desc, fi, ff, color) {
    ...
    document.getElementById('e_color').value = color || '#3730a3';
...
function abrirModalEditarTarea(id, titulo, desc, color, asigId) {
    ...
    document.getElementById('et_color').value = color || '#3730a3';
