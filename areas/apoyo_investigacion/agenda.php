...
$extraHead = '<style>
:root{--color-primary:#3730a3;--color-primary-dark:#1e1b6b;--color-primary-hover:#4338ca;}
...
.calendar-header-nav h3{margin:0;font-size:1.4rem;font-weight:700;color:#3730a3;}
...
.btn-nav:hover{background:#f8fafc;color:#3730a3;}
...
.calendar-cell.today .day-number{color:#fff;background:#3730a3;font-weight:700;}
...
.btn-evento-accion:hover{background:#fff;color:#3730a3;}
.nota-inv{background:#ede9fe;border-top:3px solid #3730a3;}
...
.tarea-card{background:#fff;border-radius:10px;box-shadow:0 2px 4px rgba(0,0,0,.04);border:1px solid #e2e8f0;padding:1.25rem;cursor:grab;border-top:5px solid #3730a3;transition:all .2s;}
...
.ag-form-control:focus{border-color:#3730a3;box-shadow:0 0 0 3px rgba(109,40,217,.12);background:#fff;outline:none;}
.mb-16{margin-bottom:16px;}
.btn-ag-primary{background:#3730a3!important;border-color:#3730a3!important;color:#fff!important;}
.btn-ag-primary:hover{background:#4338ca!important;}
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
    <div>
        <h2 style="font-weight:800;color:#0f172a;margin:0;"><i class="fa-solid fa-calendar-week" style="color:#3730a3;"></i> Agenda y Tablero de Tareas</h2>
        <p style="color:#64748b;margin:4px 0 0;">Eventos del área — Desarrollo Tecnológico y Vinculación.</p>
    </div>
    <button class="btn" onclick="agAbrirModal('modalCrearEvento')" style="background:#3730a3;border-color:#3730a3;color:#fff;">
        ...
        <?php for ($dia = 1; $dia <= $diasEnMes; $dia++):
            ...
            <div class="calendar-cell <?= $esHoy ? 'today' : '' ?>" onclick="agAbrirCrearDesdeCelda('<?= $fechaIso ?>')">
                ...
                <?php foreach ($calEvs[$dia] ?? [] as $ev):
                    ...
                    $claseNota = $esCumple ? 'nota-dorado' : ($esInstitucional ? 'nota-institucional' : 'nota-inv');
...
<div class="agenda-modal-backdrop" id="modalCrearEvento" style="display:none;" onclick="if(event.target===this)agCerrarModal('modalCrearEvento')">
    <div class="agenda-modal" style="animation:agFadeIn .25s;">
        <div class="agenda-modal-header" style="background:linear-gradient(135deg,#3730a3,#1e1b6b);">
