<?php
/**
 * COMECyT — Personal del Área (Apoyo a la Investigación Científica)
 * Solo lectura. cat_personal filtrado por cve_area = 9.
 */
...
$pdo = getConnection();
$stmt = $pdo->prepare("SELECT cve_personal,nombre,appat,apmat,puesto,correo_institucional,telefono,foto_perfil,activo FROM cat_personal WHERE cve_area=9 ORDER BY nombre,appat");
$stmt->execute();
...
.personal-header h2{font-size:1.6rem;font-weight:800;color:var(--inv-primary);margin:0;display:flex;align-items:center;gap:10px;}
.personal-header h2 i{color:var(--inv-accent);}
...
.personal-card:hover{transform:translateY(-4px);box-shadow:0 14px 30px rgba(55,48,163,.1);border-color:var(--inv-accent);}
.personal-card-header{background:linear-gradient(135deg,var(--inv-primary),var(--inv-dark,#1e1b4b));padding:24px 20px 60px;position:relative;}
...
.personal-avatar-ph{position:absolute;bottom:-36px;left:50%;transform:translateX(-50%);width:72px;height:72px;border-radius:50%;background:var(--inv-accent);display:flex;align-items:center;justify-content:center;font-size:1.8rem;color:#fff;border:3px solid #fff;box-shadow:0 4px 14px rgba(0,0,0,.15);font-weight:700;}
...
.personal-puesto{font-size:.8rem;color:#64748b;margin-bottom:14px;}
...
.personal-meta-item{display:flex;align-items:center;gap:8px;font-size:.8rem;color:#475569;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.personal-meta-item i{color:var(--inv-accent);width:14px;flex-shrink:0;}
</style>
<div class="personal-header">
    <h2><i class="fa-solid fa-id-card"></i> Personal del Área</h2>
    <span style="background:rgba(55,48,163,.06);color:#3730a3;border-radius:20px;padding:5px 14px;font-size:.82rem;font-weight:600;"><?=$totalActivo?> activos · <?=count($personal)?> total</span>
</div>
...
<?php foreach($personal as $p): $nombre=trim($p['nombre'].' '.$p['appat'].' '.$p['apmat']);$ini=mb_strtoupper(mb_substr($p['nombre'],0,1,'UTF-8')); ?>
    <div class="personal-card">
        <div class="personal-card-header" style="background:linear-gradient(135deg,#3730a3,#1e1b4b);">
