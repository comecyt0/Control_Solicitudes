<?php
/**
 * COMECyT — API: Repositorio de Documentos (Departamento Administrativo)
 * Gestión de carpetas y archivos. Solo accesible para administradores.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && $accion === 'listar_carpetas') {
    $stmt = $pdo->query(
        "SELECT id, nombre, padre_id FROM adm_repo_carpetas ORDER BY padre_id NULLS FIRST, nombre"
    );
...
if ($accion === 'crear_carpeta') {
    ...
    $chk = $pdo->prepare("SELECT id FROM adm_repo_carpetas WHERE nombre = :n AND padre_id = :p");
    $chk->execute([':n' => $nombre, ':p' => $padreId]);
    if ($chk->fetch()) repoJson(false, error: 'Ya existe una carpeta con ese nombre aquí');

    $ins = $pdo->prepare(
        "INSERT INTO adm_repo_carpetas (nombre, padre_id, creado_por) VALUES (:n, :p, :a) RETURNING id"
    );
    $ins->execute([':n' => $nombre, ':p' => $padreId, ':a' => $adminId]);
    $newId = $ins->fetchColumn();

    // Crear directorio físico
    $dir = __DIR__ . '/../../../public/uploads/adm_repo/' . $newId;
...
if ($accion === 'subir_archivo') {
    ...
    $destDir  = __DIR__ . '/../../../public/uploads/adm_repo/' . $carpetaId;
    ...
    $ins = $pdo->prepare(
        "INSERT INTO adm_repo_archivos (carpeta_id, nombre_original, archivo_path, tipo_mime, tamano_bytes, creado_por)
         VALUES (:c, :n, :p, :m, :s, :a) RETURNING id"
    );
    $ins->execute([
        ':c' => $carpetaId,
        ':n' => $nombreOrig,
        ':p' => 'adm_repo/' . $carpetaId . '/' . $uuidName,
        ':m' => $mime,
        ':s' => $tamano,
        ':a' => $adminId,
    ]);
...
if ($accion === 'eliminar_archivo') {
    ...
    $stmt = $pdo->prepare("SELECT archivo_path FROM adm_repo_archivos WHERE id = ?");
    $stmt->execute([$id]);
    $fila = $stmt->fetch();
    ...
    $pdo->prepare("DELETE FROM adm_repo_archivos WHERE id = ?")->execute([$id]);
...
if ($accion === 'eliminar_carpeta') {
    ...
    $stmt = $pdo->prepare(
        "SELECT archivo_path FROM adm_repo_archivos
         WHERE carpeta_id IN (
             WITH RECURSIVE sub AS (
                 SELECT id FROM adm_repo_carpetas WHERE id = ?
                 UNION ALL
                 SELECT c.id FROM adm_repo_carpetas c INNER JOIN sub s ON c.padre_id = s.id
             ) SELECT id FROM sub
         )"
    );
    $stmt->execute([$id]);
    ...
    $pdo->prepare("DELETE FROM adm_repo_carpetas WHERE id = ?")->execute([$id]);
...
if ($accion === 'renombrar') {
    ...
    if ($tipo === 'carpeta') {
        $pdo->prepare("UPDATE adm_repo_carpetas SET nombre = ? WHERE id = ?")->execute([$nombre, $id]);
    } elseif ($tipo === 'archivo') {
        $pdo->prepare("UPDATE adm_repo_archivos SET nombre_original = ? WHERE id = ?")->execute([$nombre, $id]);
    } else {
...
if ($accion === 'set_marcador') {
    ...
    $stmt = $pdo->prepare("UPDATE adm_repo_archivos SET marcador = ? WHERE id = ?");
