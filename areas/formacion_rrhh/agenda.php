    ...
    if ($accion === 'crear_evento') {
        ...
        if ($titulo) {
            ...
            $pdo->prepare("INSERT INTO sb_kanban_tareas (titulo, descripcion, color, estatus, creado_por, asignado_a, cve_area) VALUES (?,?,?,'pendiente',?,?,?)")
                ->execute([$titulo, trim(postParam('descripcion')), postParam('color','#0f766e'), $idAutor, $asig, $cveArea]);
        ...
        if ($id > 0 && trim(postParam('titulo'))) {
            ...
            $pdo->prepare("UPDATE sb_kanban_tareas SET titulo=?,descripcion=?,color=?,asignado_a=? WHERE id=?")
                ->execute([trim(postParam('titulo')), trim(postParam('descripcion')), postParam('color','#0f766e'), $asig, $id]);
...
$extraHead = '<style>
:root { --color-primary:#0f766e; --color-primary-dark:#134e4a; --color-primary-light:#3b82f6; --color-primary-hover:#0d9488; --color-accent:#B19A6D; }
...
.nota-rrhh{background:#ccfbf1;border-top:3px solid #0f766e;}
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
    <div>
        <h2 style="font-weight:800;color:#0f172a;margin:0;"><i class="fa-solid fa-calendar-week" style="color:#0f766e;"></i> Agenda JurÃ­dico-Administrativa</h2>
        <p style="color:#64748b;margin:0;">GestiÃ³n de eventos del Ã¡rea y tablero de tareas.</p>
    </div>
    <div style="display:flex;gap:10px;">
        <button class="btn btn-primary" onclick="abrirModalCrear()" style="background:#0f766e;border-color:#0f766e;">
        ...
        <?php for ($dia = 1; $dia <= $diasEnMes; $dia++):
            ...
            <div class="calendar-cell <?= $esHoy ? 'today' : '' ?>" onclick="abrirModalCrearDesdeCelda('<?= $fechaIso ?>','<?= $fechaIso ?>')">
                ...
                <?php foreach ($calEvs[$dia] ?? [] as $ev):
                    $colorNota  = isset($ev['es_cumple']) ? 'nota-dorado' : (isset($ev['es_institucional']) ? 'nota-institucional' : 'nota-rrhh');
    ...
    <div class="modal">
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                ...
                <div><label class="form-label">Color</label><input type="color" name="color" value="#0f766e" class="form-control" style="height:42px;padding:4px;"></div>
            </div>
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" style="background:#0f766e;border-color:#0f766e;"><i class="fa-solid fa-check"></i> Guardar</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="modal">
        ...
        <form method="POST" id="formEditarEvento">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" style="background:#0f766e;border-color:#0f766e;"><i class="fa-solid fa-check"></i> Actualizar</button>
            </div>
        </form>
    </div>
</div>
...
function abrirModalEditar(id, titulo, desc, fi, ff, color) {
    ...
    document.getElementById('e_color').value = color || '#0f766e';
...
function abrirModalEditarTarea(id, titulo, desc, color, asigId) {
    ...
    document.getElementById('et_color').value = color || '#0f766e';
