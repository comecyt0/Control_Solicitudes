    ...
    if ($accion === 'crear_evento') {
        ...
        if ($titulo) {
            ...
            $pdo->prepare("INSERT INTO sb_kanban_tareas (titulo, descripcion, color, estatus, creado_por, asignado_a, cve_area) VALUES (?,?,?,'pendiente',?,?,?)")
                ->execute([$titulo, trim(postParam('descripcion')), postParam('color','#991b1b'), $idAutor, $asig, $cveArea]);
        ...
        if ($id > 0 && trim(postParam('titulo'))) {
            ...
            $pdo->prepare("UPDATE sb_kanban_tareas SET titulo=?,descripcion=?,color=?,asignado_a=? WHERE id=?")
                ->execute([trim(postParam('titulo')), trim(postParam('descripcion')), postParam('color','#991b1b'), $asig, $id]);
...
$extraHead = '<style>
:root { --color-primary:#991b1b; --color-primary-dark:#7f1d1d; --color-primary-light:#3b82f6; --color-primary-hover:#b91c1c; --color-accent:#B19A6D; }
...
.nota-aj{background:#fff1f2;border-top:3px solid #991b1b;}
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
    <div>
        <h2 style="font-weight:800;color:#0f172a;margin:0;"><i class="fa-solid fa-calendar-week" style="color:#991b1b;"></i> Agenda JurÃ­dico-Administrativa</h2>
        ...
        <?php for ($dia = 1; $dia <= $diasEnMes; $dia++):
            ...
            <div class="calendar-cell <?= $esHoy ? 'today' : '' ?>" onclick="abrirModalCrearDesdeCelda('<?= $fechaIso ?>','<?= $fechaIso ?>')">
                ...
                <?php foreach ($calEvs[$dia] ?? [] as $ev):
                    $colorNota  = isset($ev['es_cumple']) ? 'nota-dorado' : (isset($ev['es_institucional']) ? 'nota-institucional' : 'nota-aj');
    ...
    <div class="modal">
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                ...
                <div><label class="form-label">Color</label><input type="color" name="color" value="#991b1b" class="form-control" style="height:42px;padding:4px;"></div>
            </div>
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" style="background:#991b1b;border-color:#991b1b;"><i class="fa-solid fa-check"></i> Guardar</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="modal">
        ...
        <form method="POST" id="formEditarEvento">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" style="background:#991b1b;border-color:#991b1b;"><i class="fa-solid fa-check"></i> Actualizar</button>
            </div>
        </form>
    </div>
</div>
...
function abrirModalEditar(id, titulo, desc, fi, ff, color) {
    ...
    document.getElementById('e_color').value = color || '#991b1b';
...
function abrirModalEditarTarea(id, titulo, desc, color, asigId) {
    ...
    document.getElementById('et_color').value = color || '#991b1b';
