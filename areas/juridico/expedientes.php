<?php
/**
 * COMECyT — Expedientes Jurídicos (AJ) — folio EJ-YYYY-NNNN
 */
...
$stmt=$pdo->prepare("SELECT * FROM aj_expedientes $where ORDER BY fecha_inicio DESC NULLS LAST, created_at DESC");
$stmt->execute($params);$expedientes=$stmt->fetchAll();

$tipos=['administrativo','laboral','civil','penal','otro'];
$estatuses=['activo','en_proceso','resuelto','archivado'];

$fc=getParam('flash');$flash='';$tipoF='';
if($fc==='creado'){$flash='Expediente registrado.';$tipoF='success';}
elseif($fc==='editado'){$flash='Expediente actualizado.';$tipoF='success';}
elseif($fc==='eliminado'){$flash='Expediente eliminado.';$tipoF='success';}

$pageTitle='Expedientes Jurídicos'; $activeMenu='expedientes';
...
.badge-administrativo{background:#fff1f2;color:#991b1b;}.badge-laboral{background:#fef3c7;color:#d97706;}.badge-civil{background:#ede9fe;color:#6d28d9;}.badge-penal{background:#fce7f3;color:#db2777;}.badge-otro{background:#f1f5f9;color:#475569;}
...
.modal{background:#fff;width:100%;max-width:640px;border-radius:20px;overflow:hidden;display:flex;flex-direction:column;max-height:90vh;}
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
    <div><h2 style="font-weight:800;color:#0f172a;margin:0;"><i class="fa-solid fa-folder-open" style="color:#991b1b;"></i> Expedientes Jurídicos</h2><p style="color:#64748b;margin:0;">Registro y seguimiento de expedientes del área.</p></div>
    <div style="display:flex;gap:8px;">
        <a href="api/expedientes.php?accion=exportar_csv&csrf_token=<?=$_SESSION['csrf_token']?>" class="btn" style="background:#f1f5f9;color:#475569;border:1px solid #e2e8f0;font-size:.88rem;"><i class="fa-solid fa-file-csv"></i> CSV</a>
        <button class="btn btn-primary" onclick="abrirModalCrear()" style="background:#991b1b;border-color:#991b1b;"><i class="fa-solid fa-plus"></i> Nuevo Expediente</button>
    </div>
</div>
...
<div style="display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap;">
    <form method="GET" style="display:flex;gap:10px;flex-wrap:wrap;">
        ...
        <?php if($filtroEst||$filtroTipo): ?><a href="expedientes.php" class="btn" style="background:#f1f5f9;color:#475569;border:1px solid #e2e8f0;padding:9px 14px;border-radius:10px;font-size:.88rem;">Limpiar</a><?php endif; ?>
    </form>
    <span style="margin-left:auto;align-self:center;font-size:.85rem;color:#64748b;"><?=count($expedientes)?> expedientes</span>
</div>
...
<div style="padding:70px;text-align:center;color:#94a3b8;"><i class="fa-solid fa-folder-open" style="font-size:2.8rem;opacity:.3;margin-bottom:14px;display:block;"></i><p>No hay expedientes registrados.</p></div>
<?php else: ?>
<div style="overflow-x:auto;">
<table class="aj-table">
    <thead><tr><th>Folio</th><th>Tipo</th><th>Asunto</th><th>Partes</th><th>Tribunal</th><th>Periodo</th><th>Estatus</th><th>Acciones</th></tr></thead>
    <tbody>
    <?php foreach($expedientes as $e): ?>
    <tr>
        <td><span class="folio-tag"><?=esc($e['folio'])?></span></td>
        <td><span class="badge-pill badge-<?=$e['tipo']?>"><?=ucfirst(esc($e['tipo']))?></span></td>
        <td style="max-width:180px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:.82rem;"><?=esc($e['asunto']?:'—')?></td>
        <td style="font-size:.8rem;max-width:160px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?=esc($e['partes']?:'—')?></td>
        <td style="font-size:.78rem;"><?=esc($e['tribunal']?:'—')?></td>
        <td style="font-size:.78rem;white-space:nowrap;"><?=$e['fecha_inicio']?date('d/m/Y',strtotime($e['fecha_inicio'])):'—'?> – <?=$e['fecha_cierre']?date('d/m/Y',strtotime($e['fecha_cierre'])):'—'?></td>
        <td><span class="badge-pill badge-<?=$e['estatus']?>"><?=ucfirst(str_replace('_',' ',$e['estatus']))?></span></td>
        <td>
            <button class="action-btn" onclick="abrirModalEditar(<?=htmlspecialchars(json_encode($e),ENT_QUOTES)?>)"><i class="fa-solid fa-pencil"></i></button>
            <button class="action-btn danger" onclick="eliminar(<?=$e['id']?>,'<?=esc($e['folio'])?>') "><i class="fa-solid fa-trash-can"></i></button>
        </td>
    </tr>
    <?php endforeach; ?>
    </tbody>
...
<div class="modal-backdrop" id="modalCrear"><div class="modal">
    <div class="modal-header"><h3 class="modal-title"><i class="fa-solid fa-folder-open"></i> Nuevo Expediente</h3><button class="modal-close" onclick="cerrarModal('modalCrear')"><i class="fa-solid fa-xmark"></i></button></div>
    <form method="POST" action="api/expedientes.php"><?=csrfField()?><input type="hidden" name="accion" value="crear">
    <div class="modal-body">
        ...
        <div class="mb-14"><label class="form-label">Asunto *</label><textarea name="asunto" class="form-control" rows="2" required></textarea></div>
        <div class="mb-14"><label class="form-label">Partes (actor/demandado)</label><input type="text" name="partes" class="form-control"></div>
        <div class="mb-14"><label class="form-label">Tribunal / Autoridad</label><input type="text" name="tribunal" class="form-control"></div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;" class="mb-14">
            <div><label class="form-label">Inicio</label><input type="date" name="fecha_inicio" class="form-control"></div>
            <div><label class="form-label">Cierre / Resolución</label><input type="date" name="fecha_cierre" class="form-control"></div>
        </div>
...
<div class="modal-backdrop" id="modalEditar"><div class="modal">
    <div class="modal-header"><h3 class="modal-title"><i class="fa-solid fa-pencil"></i> Editar Expediente</h3><button class="modal-close" onclick="cerrarModal('modalEditar')"><i class="fa-solid fa-xmark"></i></button></div>
    <form method="POST" action="api/expedientes.php"><?=csrfField()?><input type="hidden" name="accion" value="editar"><input type="hidden" name="id" id="ed_id">
    <div class="modal-body">
        ...
        <div class="mb-14"><label class="form-label">Asunto *</label><textarea name="asunto" id="ed_asunto" class="form-control" rows="2" required></textarea></div>
        <div class="mb-14"><label class="form-label">Partes</label><input type="text" name="partes" id="ed_partes" class="form-control"></div>
        <div class="mb-14"><label class="form-label">Tribunal / Autoridad</label><input type="text" name="tribunal" id="ed_tribunal" class="form-control"></div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;" class="mb-14">
            <div><label class="form-label">Inicio</label><input type="date" name="fecha_inicio" id="ed_fi" class="form-control"></div>
            <div><label class="form-label">Cierre</label><input type="date" name="fecha_cierre" id="ed_fc" class="form-control"></div>
        </div>
...
<form method="POST" action="api/expedientes.php" id="frmEliminar" style="display:none;"><?=csrfField()?><input type="hidden" name="accion" value="eliminar"><input type="hidden" name="id" id="del_id"></form>
<script>
...
function abrirModalEditar(e){
    document.getElementById('ed_id').value=e.id;document.getElementById('ed_folio_lbl').textContent=e.folio||'—';
    document.getElementById('ed_asunto').value=e.asunto||'';document.getElementById('ed_partes').value=e.partes||'';
    document.getElementById('ed_tribunal').value=e.tribunal||'';document.getElementById('ed_fi').value=e.fecha_inicio||'';
    document.getElementById('ed_fc').value=e.fecha_cierre||'';document.getElementById('ed_obs').value=e.observaciones||'';
    const sel=(id,val)=>{const s=document.getElementById(id);[...s.options].forEach(o=>o.selected=o.value===val);};
    sel('ed_tipo',e.tipo);sel('ed_est',e.estatus);document.getElementById('modalEditar').classList.add('active');
}
function eliminar(id,folio){if(!confirm(`¿Eliminar expediente "${folio}"?`))return;document.getElementById('del_id').value=id;document.getElementById('frmEliminar').submit();}
