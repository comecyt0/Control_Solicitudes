<?php
/**
 * COMECyT — Agenda y Tablero de Tareas (Jurídico Administrativo)
 * Calendario de eventos + Kanban de área, patrón idéntico a Difusión.
 */
    ...
    if ($accion === 'crear_evento') {
        ...
        if ($titulo) {
            ...
            $pdo->prepare("INSERT INTO sb_kanban_tareas (titulo, descripcion, color, estatus, creado_por, asignado_a, cve_area) VALUES (?,?,?,'pendiente',?,?,?)")
                ->execute([$titulo, trim(postParam('descripcion')), postParam('color','#1e3a5f'), $idAutor, $asig, $cveArea]);
        ...
        if ($id > 0 && trim(postParam('titulo'))) {
            ...
            $pdo->prepare("UPDATE sb_kanban_tareas SET titulo=?,descripcion=?,color=?,asignado_a=? WHERE id=?")
                ->execute([trim(postParam('titulo')), trim(postParam('descripcion')), postParam('color','#1e3a5f'), $asig, $id]);
...
elseif ($flashCode === 'tarea_creada')     { $mensajeFlash = 'Tarea añadida.';               $tipoFlash = 'success'; }
...
foreach ($stmtB->fetchAll(PDO::FETCH_ASSOC) as $cp) {
    ...
    $calEvs[$diaCumple][] = [
        'id' => null, 'titulo' => '🎂 '.$nombreC, 'descripcion' => 'Cumpleaños',
...
$extraHead = '<style>
:root { --color-primary:#1e3a5f; --color-primary-dark:#142845; --color-primary-light:#3b82f6; --color-primary-hover:#254a78; --color-accent:#B19A6D; }
...
.nota-ja{background:#e0eaf8;border-top:3px solid #1e3a5f;}
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
    <div>
        <h2 style="font-weight:800;color:#0f172a;margin:0;"><i class="fa-solid fa-calendar-week" style="color:#1e3a5f;"></i> Agenda Jurídico-Administrativa</h2>
        <p style="color:#64748b;margin:0;">Gestión de eventos del área y tablero de tareas.</p>
    </div>
        ...
        <?php for ($dia = 1; $dia <= $diasEnMes; $dia++):
            ...
            <div class="calendar-cell <?= $esHoy ? 'today' : '' ?>" onclick="abrirModalCrearDesdeCelda('<?= $fechaIso ?>','<?= $fechaIso ?>')">
                ...
                <?php foreach ($calEvs[$dia] ?? [] as $ev):
                    $colorNota  = isset($ev['es_cumple']) ? 'nota-dorado' : (isset($ev['es_institucional']) ? 'nota-institucional' : 'nota-ja');
    ...
    <div class="modal">
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                ...
                <div><label class="form-label">Color</label><input type="color" name="color" value="#1e3a5f" class="form-control" style="height:42px;padding:4px;"></div>
            </div>
...
function abrirModalEditar(id, titulo, desc, fi, ff, color) {
    ...
    document.getElementById('e_color').value = color || '#1e3a5f';
...
function abrirModalEditarTarea(id, titulo, desc, color, asigId) {
    ...
    document.getElementById('et_color').value = color || '#1e3a5f';
