...
- **Calendario Institucional (Sala de Juntas)**: Se implementó un sistema de reservación de salas integrado al calendario público (`public/calendario.php`). Los usuarios pueden marcar "¿Requiere Sala de Juntas?" al solicitar un evento. El sistema captura **automáticamente** el nombre (`persona_solicitante`) y área (`area_solicitante`) del usuario desde la sesión. Administrativamente, al aceptar la solicitud en `admin/calendario.php`, los datos se propagan a la tabla `eventos`. Se agregó iconografía específica (`fa-person-chalkboard`) en el grid y vistas de detalle.
