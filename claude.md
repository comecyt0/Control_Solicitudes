...
- **🚨 Nueva Política: Administrador Editorial Global (DG) 🚨**: Se ha otorgado al área de **Dirección General (Cve 2)** el rol de "Junta Editorial Global". Esto rompe el aislamiento multi-tenant en su calendario específico (`areas/direccion_general/calendario_editorial.php`), permitiéndoles visualizar, editar y eliminar cualquier registro de `df_eventos_editoriales` sin importar su `cve_area` de origen. Esta excepción es vital para la coordinación institucional centralizada.
