...
- **Panel Desarrollo Tecnológico y Vinculación**: 7 módulos: Dashboard (KPIs: proyectos activos, convenios vigentes, transferencias, tareas; tabla proyectos recientes + convenios próximos a vencer con alerta 30d), Agenda+Kanban (cve_area=12), Personal, Proyectos (`dt_proyectos` — tipo plataforma/sistema/app, líder, tecnologías como pills), Convenios (`dt_convenios` — folio `CV-YYYY-NNNN`, alerta rojo ≤30d vencimiento, CSV), Transferencias (`dt_transferencias` — patente/prototipo/licencia/spinoff, card-grid, CSV), Reportes (3 CSV). Paleta: Violet `#6d28d9`. Área: `desarrollo_tecnologico`, cve_area=12.
