...
- **Panel Formación RRHH**: 7 módulos: Dashboard (KPIs: cursos activos, becas vigentes, acciones igualdad, tareas), Agenda+Kanban (cve_area=10), Personal, Cursos (`rrhh_cursos` — modalidad, horas, cupo/inscritos, barra capacidad, CSV), Becas (`rrhh_becas` — posgrado/idioma/certificación, CSV), Igualdad (`rrhh_igualdad` — folio auto `IG-YYYY-NNNN`, CSV), Reportes (3 CSV). Paleta: Teal `#0f766e`. Área: `formacion_rrhh`, cve_area=10. Agenda generada con PowerShell sed desde `juridico_igualdad/agenda.php`.
