...
- **📅 Calendario — Desbordamiento de Celdas**: El `<div class="calendar-cell">` es un contenedor `display:flex` dentro de un `display:grid`. Sin `overflow:hidden` y `width:0` en la celda, los hijos flex se expanden al ancho del texto. La combinación `width:0 + overflow:hidden` en `.calendar-cell` fuerza al grid a repartir el espacio y constriñe el contenido. El fix global está en `assets/css/admin_extra.css` (cargado en todos los calendarios).
