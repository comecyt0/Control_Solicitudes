...
foreach ($relative in $files) {
    $fullPath = "$baseDir\$relative"
    if (-not (Test-Path $fullPath)) { continue }
    
    $content = [IO.File]::ReadAllText($fullPath, $utf8NoBom)
    $original = $content
    ...
    # Check if it already has cve_area to avoid double injection
    if ($content.Contains("INSERT INTO eventos ") -and -not $content.Contains("cve_area) VALUES (?, ?, ?, ?, ?, ?, `$publico, ?)")) {
    ...
    if ($content.Contains("INSERT INTO sb_kanban_tareas ") -and -not $content.Contains("cve_area) VALUES (?, ?, ?, 'pendiente', ?, ?, ?)")) {
        $content = $content.Replace(
